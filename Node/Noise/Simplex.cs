...
namespace NoisePerspective
{
    namespace Noise
    {
        ...
        {
            public SharpNoise.Modules.Simplex outputModule = new SharpNoise.Modules.Simplex();

            public Simplex(Scene scene, Vector2 position) : base(scene, position)
            {
                SetHeights();
                Refresh(); //Starts blank without refresh
            }
